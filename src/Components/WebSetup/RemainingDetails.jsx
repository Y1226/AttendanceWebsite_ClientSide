...
import { FillNewSeminarManagerPassword, FinishFillingOutTheDataForANewSeminar } from '../../Redux/Actions/SignInActions'
...
export const RemainingDetails = () => {
    ...
    const password = useSelector(x => x.SignInReducer.NewSeminar)
    ...
    useEffect(() => {
        async function fetchData() {
            await getPasswordLottery().then(x => dispatch(FillNewSeminarManagerPassword(x.data)))
        }
        fetchData()
    }, [dispatch])
    ...
    const sendEmail = async (e) => {
        ...
        try {
            ...
            dispatch(FinishFillingOutTheDataForANewSeminar(seminarAddress.current.value, seminarPhoneNumber.current.value, seminarFaxNumber.current.value, fileName, true))
            emailjs.sendForm('service_i7w7pvq', 'template_ktdlaq1', e.target, '6Er4YSAntswoE2pQP').then(res => console.log(res)).catch(err => console.log(err))
            // alert(password.seminarManagerPassword)
        ...
        <div className="detailsForm">
            <div className='detailsFormContent'>
                ...
                <form onSubmit={sendEmail}>
                    ...
                    <input type="text" maxLength={10} placeholder="טלפון" id="SeminarPhoneNumber" ref={seminarPhoneNumber} onKeyDown={(e) => { if (IsTheCharacterInputANumber(e)) e.preventDefault() }} /><br />
                    <input type="text" maxLength={10} placeholder="פקס" id="SeminarFaxNumber" ref={seminarFaxNumber} onKeyDown={(e) => { if (IsTheCharacterInputANumber(e)) e.preventDefault() }} /><br />
                    <input type="text" value={password.seminarManagerPassword} id="password" name='code' hidden readOnly /><br />
                    ...
                    {/* Next button */}
                    <button className="plusButton div_next" type='submit' style={{ marginBottom: '50px' }}>
